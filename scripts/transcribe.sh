...
case "$BACKEND" in
  whisper-cpp)
    ...
    for b in whisper-cli whisper-cpp; do
      command -v "$b" &>/dev/null && WHISPER_BIN="$b" && break
    done
    if [ -z "$WHISPER_BIN" ]; then
      echo "❌ whisper-cli/whisper-cpp bulunamadı. Kurmak için: brew install whisper-cpp" >&2
      exit 1
    fi
    MODEL_PATH="/opt/homebrew/share/whisper-cpp/models/ggml-base.bin"
    [ ! -f "$MODEL_PATH" ] && MODEL_PATH="$(brew --prefix)/share/whisper-cpp/models/ggml-base.bin"
    RESULT_BASE="/tmp/voice_result_$$"
    "$WHISPER_BIN" --language tr --model "$MODEL_PATH" "$INPUT" -otxt -of "$RESULT_BASE" 2>/dev/null
